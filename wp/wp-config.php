...
/**
 * Support for Neon / Vercel Postgres / TiDB automated environment variables.
 * This allows "one-click" deployment without manually entering DB credentials.
 */
if (isset($_ENV['POSTGRES_URL']) || isset($_ENV['DATABASE_URL']) || isset($_ENV['NEON_DATABASE_URL'])) {
  $db_url = $_ENV['POSTGRES_URL'] ?? $_ENV['DATABASE_URL'] ?? $_ENV['NEON_DATABASE_URL'];
  ...
  if ($parsed_url) {
    ...
    // Set SSL for Neon/Postgres
    if (str_contains($db_url, 'postgres') || str_contains($db_url, 'neon.tech')) {
...
if (!defined('DB_NAME') && isset($_ENV['DATABASE'])) {
  define( 'DB_NAME', $_ENV['DATABASE'] );
}

/** Database username */
if (!defined('DB_USER') && isset($_ENV['USERNAME'])) {
  define( 'DB_USER', $_ENV['USERNAME'] );
}

/** Database password */
if (!defined('DB_PASSWORD') && isset($_ENV['PASSWORD'])) {
  define( 'DB_PASSWORD', $_ENV['PASSWORD'] );
}

/** Database hostname */
if (!defined('DB_HOST') && isset($_ENV['HOST'])) {
